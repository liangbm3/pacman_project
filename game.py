...
# ---------------------------------------------------------------------------- #
#文件描述了PacMan游戏运作的逻辑，比如智能体的动作，配置方式，地图的坐标表示等
# ---------------------------------------------------------------------------- #
...
# ---------------------------------------------------------------------------- #
#Agent的基类，包括当前Agent的编号和getAction方法，根据相应的state作出相应的决策，返回action
# ---------------------------------------------------------------------------- #
class Agent:
    """
    ...
    def getAction(self, state):
        ...
        Agent将收到一个 GameState（来自 {pacman, capture, sonar}.py）并且必须从 Directions 返回一个动作。{北、南、东、西、停止}
        """
        raiseNotDefined()
...
class Configuration:
    ...
    def generateSuccessor(self, vector):
        ...
        通过将当前配置转换为动作向量，生成新配置。这是一个低级调用，不会尝试尊重移动的合法性。
        动作是移动向量。
        """
        x, y= self.pos
        dx, dy = vector
...
# ---------------------------------------------------------------------------- #
#AgentState类，保存agent的状态（配置，速度，害怕等）
#copy方法，对AgentState进行复制
#getPosition，获得位置
#getDirection，获得方向
# ---------------------------------------------------------------------------- #
class AgentState:
    ...
    def __init__( self, startConfiguration, isPacman ):
        self.start = startConfiguration #开始的配置
        self.configuration = startConfiguration #当前配置
        self.isPacman = isPacman    #是否为pacman
        self.scaredTimer = 0        
        self.numCarrying = 0
        self.numReturned = 0
